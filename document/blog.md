**Top 5 Practical Things You Can Do with Claude Agent Teams (Including Local Setup)**

Claude **Agent Teams** (research preview as of early 2026) is an experimental feature in **Claude Code** — Anthropic's agentic terminal/IDE tool. It lets one "team lead" Claude session spawn and coordinate multiple independent "teammate" Claude instances. These teammates work in parallel on a shared codebase or task, communicate directly via a mailbox system, manage a shared task list (with states like pending/in-progress/completed and dependencies), and collaborate without everything funneling through the lead like subagents do.

This enables true multi-perspective work: parallel research, competing debugging hypotheses, modular development (frontend/backend/tests), code reviews from specialized angles, or autonomous division of labor on large projects. It's token-heavy (each teammate loads full project context independently), so use it for tasks that benefit from parallelism over single-threaded depth.
...
### 1. Enable and Set Up Agent Teams Locally (Prerequisite for Everything Else)

Claude Code runs locally (CLI in terminal, VS Code extension, desktop app) and stores teams/tasks in `~/.claude/`.

**Steps**:
- Install Claude Code if not already (requires Claude Pro/Max/Teams/Enterprise or Console account):
  ```
  npm install -g @anthropic-ai/claude-code
  ```
  Authenticate via `claude login` (links to your Anthropic account).

- Enable the experimental flag (disabled by default):
...
- Optional but recommended for better UX (split panes per teammate):
  ...
  - Set teammate display mode if needed:
    ```json
    {
      "teammateMode": "tmux"  // or "in-process" (default, all in one terminal with Shift+Up/Down to switch)
    }
    ```
...
- Start Claude Code in your project folder:
  ...
  claude
  ```
  (Or use VS Code extension / desktop app → pick local folder.)

Teams get stored in `~/.claude/teams/{team-name}/` (config.json with members) and tasks in `~/.claude/tasks/{team-name}/`.

### 2. Spawn a Team for Parallel Code Review or Hypothesis Testing
...
Clean up the team  // after all teammates down
```

### 3. Coordinate Modular Development (Frontend/Backend/Tests in Parallel)
...
Wait for all to finish before merging/synthesizing.
```

Lead delegates, teammates claim tasks, lock files to avoid races, message for coordination (e.g., "Backend: expose this API for CLI").
...
### 4. Run Adversarial/Competing Hypothesis Investigations
...
Teammates discuss peer-to-peer → reduces bias, explores broadly.

### 5. Automate Multi-Perspective Research or Planning (Non-Code Too)
...
- Limitations: Experimental — no resume for teammates easily, one team per session, higher cost (~7x in plan mode), slow shutdown.

This is hands-on multi-agent collaboration you can run locally today — no cloud orchestration needed beyond your Anthropic sub. Experiment in a throwaway project first.